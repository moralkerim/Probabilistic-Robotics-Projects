...
%Initial beliefs
mu_t1 = [0; 0; 0];
sigma_t1 = [0.05, 0, 0; 0, 0.05, 0; 0, 0, 0.05];
...
for i = 1:20
    ...
    [mu_t, sigma_t, X] = ukf_prediction(mu_t1, sigma_t1, d, alpha);
    ...
    for j = 1:length(signature)
        ...
        if r <= range && phi <= bearing/2 && phi >= -bearing/2
            ...
            [mu_t, sigma_t] = ukf_correction(mu_t, sigma_t, X, d, alpha, landmark(j, 1), landmark(j, 2), signature(j), r, phi, sign, er, ep, es);
