function [scale, cx, cy, ps] = error_ellipse(varargin)
...
if r==3 & c==3
  ...
  [x1,y1,z1] = getpoints(Cxy,prop.clip);
  h1=plot3(x0+k*x1,y0+k*y1,z0+k*z1,prop.style);hold on
  [y2,z2,x2] = getpoints(Cyz,prop.clip);
  h2=plot3(x0+k*x2,y0+k*y2,z0+k*z2,prop.style);hold on
  [z3,x3,y3] = getpoints(Czx,prop.clip);
  h3=plot3(x0+k*x3,y0+k*y3,z0+k*z3,prop.style);hold on
